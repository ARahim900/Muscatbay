...
| Water — Irrigation | `/water` (Irrigation tab) | 🟡 Built 2026-09-08, awaiting migration + owner review | `irrigation_meters` + `irrigation_daily_readings` (new, separate from the potable tables) | Fourth `SegmentedControl` mode. A hand-read network with no Grafana feed: 12 meters transcribed from the Kalhat sheet — three sources into the main irrigation tank (bore well, TSE from the STP, PO line), the tank outlet, five zone tanks (02 CP · 03 Z5 · 04 Z8 · 05 FM · 06 VS), controllers 08/09 and the JMB tank feed. Operators type the **cumulative meter index** for a date (`DateStepper`, capped at today); consumption is today − yesterday and stays "—" until both days exist. Page: `StatsGrid` (into main tank / outlet / zone tanks & controllers / days recorded — all `null`-honest, month sums cover derivable days only) → entry card → month ledger (meters × days, CSV) → three-line daily-flow chart (`ChartFrame`, gaps not joined). Writes go through the Server Action `saveManualReadingsAction` with the caller's own session, so RLS decides (operator+ may write, viewer sees the same card read-only). Own fetch + realtime channel, reports into the page's single `StatusChip`; a failed monthly read never blocks it. **Assumption to confirm with the owner:** the Kalhat sheet holds meter *index* readings, not daily consumption — if it holds consumption, the derivation step is removed, nothing else changes. Not yet done: migration `20260908_manual_meter_readings.sql` applied to the live project; `npm run screenshots` (needs a running app + login) |
| Water — Hand Readings (potable) | `/water` (Daily → Hand Readings tab) | 🟡 Built 2026-09-08, awaiting migration + owner review | `water_manual_meters` + `water_manual_readings` → trigger → `water_daily_consumption` | Sixth Daily tab. The eight potable bulk meters Kalhat reads by hand: Main Bulk C43659, the six zone bulks (3A 4300343 · 3B 4300344 · 5 4300345 · 8 4300342 · VS 4300335 · FM 4300346) and "Central park" = the existing Irrigation Tank 02 meter 4300320 (owner confirmation). Grafana never reports C43659, 4300342 or 4300320 (the month initializer already calls them EMAIL_OWNED) and occasionally misses 3B. The hand index is stored as recorded; the trigger `water_manual_readings_apply` derives today − yesterday into the day cell of the wide daily table, so Zone Watch, the loss balance, the Monthly month-to-date fallback, Satellite and the dashboard all pick it up with **no read-path change**. Overwrite rules (asserted in `sql/tests/manual-readings.test.sql`): `manual_owned` meters always rewrite their cell (corrections propagate, a cleared reading empties it); the other bulks fill an **empty** cell only, or one that still holds the value this table wrote (`applied_consumption`) — an instrumented Grafana value is never overwritten. Negative differences are written as-is and flagged, never clamped. Tab is reachable even for a month with no Grafana rows yet — that is exactly when it is needed. Same entry / ledger cards and Server Action as Irrigation |
...
- **Hand readings (added 2026-09-08, migration `20260908_manual_meter_readings.sql`,
  not yet applied live):** Kalhat's daily index readings for the eight potable bulk
  meters land in `water_manual_readings`; the trigger derives each day's
  consumption into the matching `water_daily_consumption.day_N` cell. Three
...
- **Open 2026-09-08 — hand readings: two things only the owner can settle.**
  (1) Whether the Kalhat sheets carry cumulative meter *indexes* (assumed; the
  app derives consumption as today − yesterday and shows the previous index
  beside every field) or daily *consumption* (then the derivation is removed
  and the typed value is written directly — a small change to
  `functions/api/manual-readings.ts` and the trigger). (2) The migration must be
  applied to the live Supabase project and the entering colleague's
  `profiles.role` set to `operator` — the default `viewer` role sees the entry
  cards read-only by design. Screenshots (`npm run screenshots`) were not
  produced in the build session (no running app / login available); check
  light and dark at 1440 px on the Vercel preview before merging.
