...
| Water — Hand Readings | `/water` (Hand Readings mode) | 🟡 Built 2026-09-08, restructured the same day on owner feedback; awaiting migration + owner review | `irrigation_meters` + `irrigation_daily_readings` (new, separate) · `water_manual_meters` + `water_manual_readings` → trigger → `water_daily_consumption` | Fourth `SegmentedControl` mode holding **two separate tables under `Tabs`: Irrigation and Potable water**, both recorded **per day** (owner ruling 2026-09-08: one new section, two tables, daily entry — the first cut had put the potable table under the Daily view). One `DateStepper` (capped at today) serves both tabs because Kalhat records both systems on the same day. Each tab: `StatsGrid` of the **selected day** (three meter groups + "meters read n / N", month-to-date in the subtitle, all `null`-honest) → entry card (cumulative meter index per meter; consumption = today − yesterday, "—" until both exist; previous index shown beside every field) → daily log (meters × every day of the month, CSV; a missed day is a visible gap) → three-line daily-consumption chart (`ChartFrame`, gaps not joined). **Irrigation:** 12 meters from the Kalhat sheet — bore well / TSE / PO line into the main tank, the tank outlet, tanks 02 CP · 03 Z5 · 04 Z8 · 05 FM · 06 VS, controllers 08 / 09, JMB feed; groups = into main tank / outlet / zone tanks & controllers. Never touches the potable tables. **Potable water:** the eight bulk meters Kalhat reads by hand — Main Bulk C43659, zone bulks 3A 4300343 · 3B 4300344 · 5 4300345 · 8 4300342 · VS 4300335 · FM 4300346, and "Central park" = the existing Irrigation Tank 02 meter 4300320 (owner confirmation); groups = Main Bulk / zone bulks / Central Park. Grafana never reports C43659, 4300342 or 4300320 (the month initializer already calls them EMAIL_OWNED) and occasionally misses 3B. The trigger `water_manual_readings_apply` derives today − yesterday into the day cell of the wide daily table, so Zone Watch, the loss balance, the Monthly month-to-date fallback, Satellite and the dashboard pick it up with **no read-path change**. Overwrite rules (asserted in `sql/tests/manual-readings.test.sql`): `manual_owned` meters always rewrite their cell; the other bulks fill an **empty** cell only, or one still holding the value this table wrote (`applied_consumption`) — an instrumented Grafana value is never overwritten. Negatives written as-is and flagged. Writes go through `saveManualReadingsAction` with the caller's own session, so RLS decides (operator+ may write; viewer sees the cards read-only with a caption). Own fetch + realtime channel per tab, reporting into the page's single `StatusChip`; a failed monthly read never blocks it. **Assumption to confirm with the owner:** the Kalhat sheets hold meter *index* readings, not daily consumption — if consumption, the derivation step is removed, nothing else changes. Not yet done: migration `20260908_manual_meter_readings.sql` applied live; `npm run screenshots` (needs a running app + login) |
